...
public class IBEATimeLimited extends Algorithm {
     ...
     */
    public IBEATimeLimited(Problem problem, long maxRunTimeMS, GenerateReports gr) {
        super(problem);
...
//    } // removeWorst
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        long elapsed = 0, last = 0;

        int populationSize, archiveSize, maxEvaluations, evaluations;
        Operator crossoverOperator, mutationOperator, selectionOperator;
        SolutionSet solutionSet, archive, offSpringSolutionSet, save;
        ...
        save = ((SolutionSet) save).union(solutionSet);
...
//        }
        ...
        while (elapsed < this.maxRunTimeMS) {
            ...
            //Compute reports
            double[][] front = new double[union.size()][];
            double[][] front2 = new double[save.size()][];
            int cpt = 0;
            Hypervolume h = new Hypervolume();
            Hypervolume h2 = new Hypervolume();
            
            for(int i=0; i<union.size(); i++){
            	reports.addObjectiv(union.get(i).getObjective(0) + " " + union.get(i).getObjective(1) + " " + union.get(i).getObjective(2) + " "
            			+ union.get(i).getObjective(3) + " " + union.get(i).getObjective(4) + "\n");
            	if(union.get(i).getObjective(0) == 0.0){
            		cpt++;
            	}
            	front[i] = new double[union.get(0).getNumberOfObjectives()];
            	for(int j=0; j<union.get(i).getNumberOfObjectives(); j++){
            		front[i][j] = union.get(i).getObjective(j);
            	}
            }
            for(int i=0; i<save.size(); i++){
            	front2[i] = new double[save.get(0).getNumberOfObjectives()];
            	for(int j=0; j<save.get(i).getNumberOfObjectives(); j++){
            		front2[i][j] = save.get(i).getObjective(j);
            	}
            }
            reports.addObjectiv("\n");
            reports.addHypervolume(h.calculateHypervolume(front, union.size(), union.get(0).getNumberOfObjectives()) + "\n");
            reports.addBigHypervolume(h2.calculateHypervolume(front2, save.size(), save.get(0).getNumberOfObjectives()) + "\n");
            reports.addNonViolated(cpt + "\n");
            reports.addTime(elapsed + "\n");
...
//        }
        ...
        //System.out.println("Save: " + save.size());

        Ranking ranking = new Ranking(archive);
        return ranking.getSubfront(0);
    } // execute
    
    /*public void filter(SolutionSet s1){
    	for(int i=0; i<s1.size(); i++){
    		for(int j=i+1; j<s1.size(); j++){
    			if(S1DominateS2(s1.get(i), s1.get(j))){
    				s1.remove(j);
    				j--;
    			} 
    			else if(S2DominateS1(s1.get(j), s1.get(i))) {
    				s1.remove(i);
    				i--;
    				break;
    			}
	...
	public boolean S1DominateS2(Solution s1, Solution s2){
		boolean result = true;
	...
	public boolean S2DominateS1(Solution s1, Solution s2){
		boolean result = true;
		for(int i=0; i<s1.getNumberOfObjectives(); i++){
			if(s1.getObjective(i) < s2.getObjective(i)){
				result = false;
