    ...
    public static void main(String[] args) throws Exception {
		...
		try {
			if(args.length<3)
			{
				...
				System.out.println(" java PATH SATIBEA filename.dimacs timeLimit outputfilename [seedFile]");
			}
            ...
            //String init_pop = fm + ".result";
            int timeLimit= Integer.parseInt(args[1]);
            String outputfilename = args[2];
            
            

            GenerateReports reports = new GenerateReports(outputfilename);
            ...
            if(args.length>=4)
            {
                ...
                //with seeds
                p = new SATIBEA_Problem(fm, augment, mandatory, dead, seedFile);
            }
            else
            {
                System.out.println("without seed");
                //without seeds
                p = new SATIBEA_Problem(fm, augment, mandatory, dead);
            ...
            Algorithm a = new SATIBEA_SettingsIBEA(p).configureSATIBEA(timeLimit, fm, ((SATIBEA_Problem) p).getNumFeatures(), ((SATIBEA_Problem) p).getConstraints(), reports);
            
            SolutionSet pop = a.execute();
            
            File f = new File(outputfilename+"_final");
            //f.getParentFile().mkdirs();
            FileWriter fw = new FileWriter(f);
            
            for (int i = 0; i < pop.size(); i++) {
                Variable v = pop.get(i).getDecisionVariables()[0];
                fw.write(((Binary)v).toString() + "\n");
            }
            ...
            for (int i = 0; i < pop.size(); i++) {
                for (int j = 0; j < pop.get(i).getNumberOfObjectives(); j++) {
                    fw.write(pop.get(i).getObjective(j) + " ");
                }
                fw.write("\n");
            }
        	...
        	e.printStackTrace();
            System.out.println("Usage: java PATH SATIBEA filename.dimacs timeLimit outputfilename [seedFile]\nThe .augment, .dead, .mandatory and .richseed files should be in the same directory as the FM.");
        }
