...
public class SATIBEA_BinarySolution extends BinarySolutionType {
    ...
    int n = 0;
    private ArrayList<ArrayList<Integer>> seed;
    ...
    //without seed
    public SATIBEA_BinarySolution(Problem problem, int nFeat, String fm, List<Integer> mandatoryFeaturesIndices, 
    		List<Integer> deadFeaturesIndices) {
        super(problem);
        this.fm = fm;
        this.nFeat = nFeat;
        ...
        //this.seed = seed;
        this.seed = new ArrayList<ArrayList<Integer>>();
    }
    
    //with seed
    public SATIBEA_BinarySolution(Problem problem, int nFeat, String fm, List<Integer> mandatoryFeaturesIndices, 
    		List<Integer> deadFeaturesIndices, ArrayList<ArrayList<Integer>> seed) {
        super(problem);
        this.fm = fm;
        this.nFeat = nFeat;
        this.mandatoryFeaturesIndices = mandatoryFeaturesIndices;
        ...
        if(n<seed.size()){
        	for (int i = 0; i < vars.length; i++) {
                ...
                for (int j = 0; j < bin.getNumberOfBits(); j++) {
                    bin.setIth(j, seed.get(n).get(j) > 0 ? true : false);
                }
            	vars[i] = bin;
        	}
        	n++;
